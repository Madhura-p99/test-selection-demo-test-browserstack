...
public class SearchFunctionalityTest extends BaseTest {
    ...
    public void testSearchExecutionWithValidSearchTerms() {
        ...
        for (String searchTerm : searchTerms) {
            ...
            // Verify either search results heading or no results message is displayed
            ...
            Assert.assertTrue(hasResults || hasNoResultsMessage,
                    "Either search results heading or 'no results found' message should be displayed for term: " + searchTerm);

            if (hasResults) {
                String headingText = searchPage.getSearchResultsHeadingText();
                Assert.assertTrue(headingText.contains(searchTerm),
                        "Search results heading should contain the search term: " + searchTerm);
                logger.info("✅ Search results heading displayed with term: '{}'", searchTerm);
            } else if (hasNoResultsMessage) {
                String messageText = searchPage.getNoResultsMessageText();
                Assert.assertTrue(messageText.contains(searchTerm),
                        "No results message should contain the search term: " + searchTerm);
                logger.info("✅ No results message displayed with term: '{}'", searchTerm);
            }
